...
contract MetaborgDistributionERC1155_V2 is ERC1155Upgradeable {
    ...
    event addElementOnDistributionEvent(uint IDDistribution, address to, uint quantity);

    mapping(uint => mangaDistributionStruct) mangaDistribution;
    mapping(uint => string) IPFSOverrideConnectionURI;
    mapping(uint => bool) reservedDistribution;
    ...
    function getRandomMangaVersion(uint _mangaDistributionID) private returns(uint){
        ...
        uint randomMangaVersionID = thresoldMappingMangaVersion(getRandom(netSupply), diamondSupply, goldSupply, diamondAssigned, goldAssigned);
    ...
    function createMangaDistribution(uint _price, uint _diamondSupply, uint _goldSupply, uint _originalSupply, address[] memory _allowedAddressToMint, uint[] memory _quantityList, uint _ERC1155DiamondID, uint _ERC1155GoldID, uint _ERC1155OriginalID, string[] memory IPFSOverrideConnectionList, bool _reservedDistribution) public onlyOwner returns(uint){
        require(_diamondSupply.add(_goldSupply).add(_originalSupply) > uint(0), "CANT_SET_NULL_DISTRIBUTION");
        ...
        require(sumQuantity == _diamondSupply.add(_goldSupply).add(_originalSupply), "QUANTITY_LENGTH_DISMATCH");
        ...
        IPFSOverrideConnectionURI[_ERC1155DiamondID] = IPFSOverrideConnectionList[0];
        IPFSOverrideConnectionURI[_ERC1155GoldID] = IPFSOverrideConnectionList[1];
        IPFSOverrideConnectionURI[_ERC1155OriginalID] = IPFSOverrideConnectionList[2];
        mangaDistributionIndex = mangaDistributionID.add(1);
        if(_reservedDistribution)  reservedDistribution[mangaDistributionID] = true;
    ...
    function mintRandomManga(uint _mangaDistributionID) public payable returns(uint){
        ...
        require(!reservedDistribution[_mangaDistributionID], "RESERVED_DISTRIBUTION");
    ...
    function addElementOnDistribution(uint _distributionID, address _address, uint _quantity) public onlyOwner returns(bool){
        mangaDistribution[_distributionID].originalSupply = mangaDistribution[_distributionID].originalSupply.add(_quantity);
        mangaDistribution[_distributionID].allowedAddressToMint[_address] = mangaDistribution[_distributionID].allowedAddressToMint[_address].add(_quantity);
        emit addElementOnDistributionEvent(_distributionID, _address, _quantity);
        return true;
    }
    ...
    function getReservedDistribution(uint _mangaDistributionID) public view returns(bool){
        return reservedDistribution[_mangaDistributionID];
    }
